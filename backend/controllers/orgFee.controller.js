  ...
  try {
    ...
    let query = `
      SELECT f.fee_id, f.fee_name, m.member_name, f.amount, f.due_date, f.payment_date,
             ...
             CASE
               WHEN f.payment_date IS NULL AND CURRENT_DATE > f.due_date THEN 'Late'
               WHEN f.payment_date IS NULL AND CURRENT_DATE <= f.due_date THEN 'On Time'
...
export const addOrgFee = async (req, res) => {
  try {
    ...
    await db.query(
      ...
      [fee_name, amount, due_date, sem_issued, acad_year_issued, member_id, orgId]
    );

    return res.status(201).json({ success: true, message: 'Fee added successfully' });
  } catch (error) {
...
export const updateOrgFeeDue = async (req, res) => {
  try {
    ...
    const result = await db.query(
      `UPDATE fee SET due_date = $1 WHERE fee_id = $2 AND org_id = $3`,
      [due_date, feeId, orgId]
    );
    ...
    return res.status(200).json({ success: true, message: 'Fee due date updated successfully' });
  } catch (error) {
...
export const deleteOrgFee = async (req, res) => {
  try {
    ...
    const result = await db.query(
      `DELETE FROM fee WHERE fee_id = $1 AND org_id = $2`,
      [feeId, orgId]
    );
...
export const getOrgFeeStats = async (req, res) => {
  try {
    ...
    const highestDebtResult = await db.query(
      `SELECT m.member_name, SUM(f.amount) as total_debt
       FROM fee f
       JOIN member m ON f.member_id = m.member_id
       WHERE f.org_id = $1 AND f.payment_date IS NULL
    ...
    const total = Number(paid_count) + Number(unpaid_count);
    const paidRatio = total ? (Number(paid_count) / total) : 0;
    const unpaidRatio = total ? (Number(unpaid_count) / total) : 0;

    return res.status(200).json({
      highestDebt: { member_name: highestDebt.member_name, total_debt: highestDebt.total_debt },
