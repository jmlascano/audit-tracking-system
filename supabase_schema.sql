-- =============================================================================
-- Batis — Supabase / PostgreSQL schema
...
-- Shared by member_part_of_org.acad_sem and fee.sem_issued
CREATE TYPE acad_sem_type AS ENUM ('1', '2', 'm');

CREATE TYPE membership_status AS ENUM ('Active', 'Inactive', 'Suspended', 'Expelled', 'Alumni');
...
CREATE TABLE org_event (
  ...
  CONSTRAINT orgevent_orgid_fk FOREIGN KEY (org_id) REFERENCES org (org_id)
);

-- acad_year is a 4-digit code like 2425, 2324, 2223
-- batch is an org-defined cohort name e.g. "float(inf)", "hash", "yb15"
CREATE TABLE member_part_of_org (
  member_id INT,
  org_id    INT,
  batch     VARCHAR(255),
  acad_year INT,
  acad_sem  acad_sem_type,
  committee VARCHAR(255),
  role      VARCHAR(255),
  status    membership_status,
  PRIMARY KEY (member_id, org_id, acad_year, acad_sem),
  CONSTRAINT memberpartoforg_memberid_fk FOREIGN KEY (member_id) REFERENCES member (member_id),
  CONSTRAINT memberpartoforg_orgid_fk    FOREIGN KEY (org_id)    REFERENCES org    (org_id)
);
...
CREATE TABLE fee (
  ...
  amount           DECIMAL(10, 2),
  due_date         DATE,
  sem_issued       acad_sem_type,
  acad_year_issued INT,
  payment_date     DATE,
  member_id        INT,
  org_id           INT,
...
-- Links a Supabase Auth user to a member or org row
CREATE TABLE user_profiles (
  auth_user_id UUID PRIMARY KEY REFERENCES auth.users (id) ON DELETE CASCADE,
  user_type    TEXT NOT NULL CHECK (user_type IN ('member', 'org')),
  app_user_id  INT  NOT NULL  -- member_id or org_id depending on user_type
);
